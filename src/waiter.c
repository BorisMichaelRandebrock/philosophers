...
/*   Updated: 2024/05/23 17:55:07 by brandebr         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
//	printf("Time: %-10ld %ld has put down left fork ⋔\n", gettime() - philo->table->start_dinner, philo->id);
	mutex_handle(&philo->right_fork->fork, UNLOCK);
//	printf("Time: %-10ld %ld has put down right fork ⋔\n", gettime() - philo->table->start_dinner, philo->id);
	philo_thinks(philo, true);
}
...
void	*wait_dinner(void *data)
{
	...
	table = (t_table *)data;
	while (!all_threads_created(&table->table_mutex,
			&table->numb_threads_runing, table->number_of_philosophers))
			;
	while (!dinner_finished(table))
	{
		i =-1;
		while (++i < table->number_of_philosophers && !dinner_finished(table))
		{
